...
const GradientButton = ({ text, ...props }: GradientButton) => {
  ...
  return (
    <div className={`bg-customGradient rounded-[50px] p-[1px] ${isDisabled?'opacity-50 cursor-not-allowed':''}`}>
      <div className={`bg-white p-[2px] rounded-[50px] ${isDisabled?'cursor-not-allowed':''}`}>
        <button
          {...props}
